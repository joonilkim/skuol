...
export default function(el){
  ...
  const _children = {}
  children.forEach(el => _children[el._id] = el)

  const remove = function(idx){
    delete _children[children[idx]._id]
  ...
  const findIndex = function(node, fromIdx=0){
    if(!_children[node._id]) return -1
  ...
  this.removeLast = function(){
    ...
    el.removeChild(node)
    delete _children[node._id]
  }

  this.hasChild = function(node){
    return node._id in _children
  }
